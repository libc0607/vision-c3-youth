...
public:
             ...
             bool enableDecodeMultiTask, bool enableDrawMultiTask, bool useBigEndian)
  {
    ...
    if (!_read_buf)
    {
      _read_buf = (uint8_t *)malloc(READ_BUFFER_SIZE);//Serial.println(F("malloc2"));
    }
    if (_read_buf)
    {
    ...
    if (_enableDrawMultiTask)
    {
      ...
      else
      {
        ...
        _pDrawTask.xqh = _xqh;
        _pDrawTask.drawFunc = pfnDraw;
        xTaskCreatePinnedToCore(drawTask, "drawTask", 2048, &_pDrawTask, 2, &_drawTask, 0);
      }
    }
    else
    {
