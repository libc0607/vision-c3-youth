...
public:
	...
	inline bool openFFat(void); // Returns true if open succeeded
  inline bool openSPIFFS(void); // Returns true if open succeeded
  inline bool openLittleFS(void); // Returns true if open succeeded
...
bool IniFile::openFFat(void)
{
	if (_file)
		_file.close();
	_file = FFat.open(_filename, _mode);
	if (isOpen()) {
		_error = errorNoError;
		return true;
	}
...
bool IniFile::openLittleFS(void)
{
 if (_file)
    _file.close();
